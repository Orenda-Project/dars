...
**D-73: Breakdown and holiday routers use `get_current_org` instead of `require_admin` (X-Admin-Token). Supersedes D-66 for these routers on 2026-05-18.** *Rationale:* D-66 introduced `X-Admin-Token` as a shared-secret env var when there was no per-org admin concept. Phase 5 added per-admin sessions (`X-Admin-Session`) issued by `/api/v1/admin/login`. Keeping two parallel auth models meant the dashboard 403'd on every breakdown/holiday endpoint because `require_admin` only honored `X-Admin-Token`. Solution: both routers now take `get_current_org`, which accepts either `X-API-Key` or `X-Admin-Session` and resolves to the calling org. Access is scoped to the caller's org: `list_breakdowns` returns global+own-org+own-class rows only; every path-bound endpoint validates that `org_id`/`school_id`/`cst_id` belongs to the caller. `require_admin` itself stays as a primitive for any future cross-org operator-only action (no v2 routers currently use it). Tests now use the demo org's API key in place of `X-Admin-Token`. *Apply:* immediately. *Decided:* PR for fix/dashboard-403 follow-up, 2026-05-18.

**D-74: 1 breakdown slot = 1 teaching day.** Pre-D-74 model treated each slot as a "curriculum activity" that could span multiple days (e.g. a 27-teaching-day chapter had 7 slots: 4 lessons, 2 assessments, 1 revision). This broke teacher mental model — "what am I teaching today?" was unanswerable from slot count alone. New model: every slot is exactly one day. Consequences: (a) `count(breakdown_slots) == breakdowns.total_teaching_days` is an invariant the planner guarantees; (b) consecutive same-topic lesson slots share `lp_type` (Identical, not phased — keeps the data model simple); (c) auto-build's chapter allocation now decomposes `chapter_days` into `lesson_days + fa_count + sa_count + revision_count`, with lesson_days distributed uniformly across the chapter's topics; (d) the projector becomes a trivial 1:1 walk over working days, anchors still pin specific slots. Cost is *explicitly not a constraint* per user — we generate freely. Migration: a one-shot SQL (`20260520000000_invalidate_pre_d74_breakdowns.sql`) soft-deletes the demo org's old breakdowns + drops realized class slots; the seed's idempotent re-run rebuilds them with the new shape. *Open questions on day allocation* answered as: uniform-per-topic split (not proportional to text length, no curator-set per-topic days). *Apply:* immediately. *Decided:* this PR, 2026-05-19.
