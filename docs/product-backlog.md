...
- **Auto-fork breakdown on CST create.** When a CST is created via `POST /api/v1/csts`, the server now auto-forks the breakdown chain so the class has a schedule out of the box: prefer an existing published org-scope breakdown → fork to class scope → publish → realize. Falls back to forking from global if no org one exists. Best-effort: failure of the auto-fork doesn't fail CST creation; the teacher just sees a "Reach out to your administrator" hint. Also: all teacher-app "missing data" empty states now point at the administrator. 2026-05-19.
