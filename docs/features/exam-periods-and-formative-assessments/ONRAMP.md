...
| Phase 3 — FA → Exam Generator inputs (`05-...`) | 🟡 In review | PR pending | Built F-3.1..F-3.4 on `feat/exam-periods-fa-phase-3`; **root Claude fixed a latent broken-join bug** (D-18) the agent inherited; 260 passed / 60 skipped; webapp typecheck/build clean; awaiting PR + deploy |

**Next thing to do:** open the Phase 3 PR against `staging`; after merge watch both deploys (Phase 3 touches webapp too, so its deploy should build, not skip). That closes the feature — mark all phases ✅ and move the entry to Closed in `docs/features/README.md`.
