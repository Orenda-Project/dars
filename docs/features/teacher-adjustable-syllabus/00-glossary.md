...
| **Action 2 — Break it down** | The already-shipped `generate_chapter_plan`: for a chapter in the path, period count = real teaching periods in its date range (`compute_teaching_days`), generate LP + assessment slots, show the teacher the LPs they'd teach. Reused unchanged; keys off a class-path chapter. (D-5) |
