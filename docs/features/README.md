...
- [teacher-adjustable-syllabus](teacher-adjustable-syllabus/README.md) — the global syllabus becomes **advisory**; each class owns its **own ordered chapter path** (`class_chapters`). Teacher picks which chapters to teach (recommended from the global), sets dates, reorders (started chapters locked), then breaks a chapter down (existing `/plan` flow, reused). Chapter status derived from taught slots. Generation/assessment content explicitly out of scope (that's `intelligent-chapter-planner`). Shipped 2026-06-03 via PR #109 (both phases).
