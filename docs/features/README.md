...
- [chapter-planner-engine](chapter-planner-engine/README.md) — **standalone** LLM-powered Chapter Planning Engine (like UG_LP/UG_EG) at `dars/chapter-planner-app/`: input a Chapter (topics→SLOs) + period count, output an ordered list of Plan Units (each a spec for one downstream UG_LP request — `lp_type` + topic text + target SLOs), ensuring full SLO coverage. LLM-only (no fallback), Agents-SDK OAuth backend for dev. Not wired into dars yet; eventually replaces the deterministic `make_chapter_plan` (D-9). Phase 1 in flight.
