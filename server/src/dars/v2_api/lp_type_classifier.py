"""
Claude-backed lp_type classifier for sub-SLOs.

Ported verbatim from `scripts/lp_type_classifier.py` (core-book-import D-4) so
the import service can run inside the app process. Given a parent SLO statement
+ a sub-SLO statement, returns one of:
...
- Prompt caching: the system block (enum definitions + few-shot examples) is
  marked cacheable, so a batch run of ~70 sub-SLOs reuses the prefix.
...
def classify_lp_type(
    *,
    ...
    if candidate in VALID_LP_TYPES:
        log.debug("classify_lp_type -> %s", candidate)
        return candidate
...
def classify_all(
    ...
    *,
    ...
    log.info("classify_all: %d records classified", len(out))
    return out
