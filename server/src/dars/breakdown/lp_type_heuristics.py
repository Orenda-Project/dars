...
Picks an lp_type for a topic based on:
  1. The parent SLO's `recommended_lp_type` (strongest signal — set
     deliberately by curriculum authors).
  2. Keyword matching against topic title + first 200 chars of topic_text.
  3. Default to subject-specific safe fallback ('reading' for Eng/Urdu,
...
def pick_lp_type(
    *,
    ...
    Returns a value that is guaranteed valid for `subject_code` (per
    lp_types.LP_TYPES_BY_SUBJECT). Never returns None; falls back to the
    per-subject default if nothing matches.
    """
    # 1. The SLO author's intent wins if it's valid for the subject.
    if recommended_lp_type and is_valid_lp_type(subject_code, recommended_lp_type):
        return recommended_lp_type

    # 2. Keyword heuristic on title + first 200 chars of topic_text (D-68).
    ...
    # 3. Per-subject default; double-check it's in the valid set.
    fallback = _DEFAULT_LP_TYPE.get(subject_code, "revision")
    if not is_valid_lp_type(subject_code, fallback):
        # Subject we don't know — degrade to whatever revision means there.
