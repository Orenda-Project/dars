"""
CPE planner core (F-2.3): call the LLM, parse strict JSON, validate, build the
ChapterPlan. No repair, no retry, no fallback (D-2) — a bad plan fails loudly.

PlanValidator enforces the D-8 invariants (a)-(e).
"""
import json
...
from models import ChapterPlan, PlanRequest, PlanUnit
from planner_llm import PlannerLLM, PlannerLLMError
...
def _extract_json(raw: str) -> str:
    ...
    repair/retry (D-2): we read what the model returned, we don't ask again or
    synthesise a plan. Models occasionally wrap valid JSON in a sentence or a
    fence (see intelligent-chapter-planner PLANNER_REPORT.md); a brace-scan
...
async def make_chapter_plan(request: PlanRequest, llm: PlannerLLM) -> ChapterPlan:
    ...
    PlanValidationError (failed invariant). No fallback (D-2).
    """
    logger.info(
