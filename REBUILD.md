...
| Phase 2 (Breakdown Engine) | 🟡 bead `feat-v2-phase-2-breakdown` open; F2.1..F2.5 ✅ shipped; F2.6 in flight (PR pending) |
...
F2.6 PR pending review/merge to `staging`. Once merged, proceed with F2.7 (Fork endpoints: org-fork-from-global, class-fork-from-org) per [docs/plans/2026-05-15-dars-v2-rebuild/04-phase-2-breakdown-engine.md](docs/plans/2026-05-15-dars-v2-rebuild/04-phase-2-breakdown-engine.md). Branch off `staging`. Reminder: features are tackled in numeric order; small/adjacent features may be bundled, but never skip ahead in the sequence.
