...
- **Symbol lookup ("where is X defined" / "which files reference Y" / "find all callers of Z"): use `graphify query "where is <symbol> defined"` instead of `grep -rl <symbol>` or `find . -name "*.py" -exec grep`.** Keep grep/find only for literal text in a *known* path (e.g. "find this exact log line in router_webhooks.py"). A PreToolUse hook will nudge if a grep-rl/find-grep symbol search runs while the graph exists.
